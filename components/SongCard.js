...
export default function SongCard({
  ...
  // 16 waveform bars with varied heights
  const waveformHeights = [
    35, 60, 45, 80, 100, 70, 50, 85, 90, 65, 40, 75, 95, 60, 50, 70,
  ];

  return (
    <div
      className={`group relative flex flex-col gap-3 p-3.5 sm:p-4 rounded-[14px] border transition-all duration-200 ${
        isSelected
    ...
    >
      {/* Top Row: Index + Artwork + Title/Artist + Controls */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        {/* Left: Index + Artwork + Meta */}
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2.5 sm:gap-3 flex-shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-[var(--line)]">
      ...
      {/* Interactive Waveform Scrubber (Active when playing or hovered) */}
      <div
        onClick={handleWaveformClick}
        className="w-full h-5 rounded-md bg-[var(--bg)]/50 hover:bg-[var(--bg)] border border-[var(--line)]/50 px-2 flex items-center gap-1 cursor-pointer transition-colors group/wave"
        title="Click to scrub 30-second preview"
      >
        <div className="flex items-center gap-[2.5px] w-full h-3.5">
          {waveformHeights.map((h, i) => {
            const barFraction = (i + 1) / waveformHeights.length;
            const isPlayed = isPlaying && barFraction <= audioProgress;
            return (
              <div
                key={i}
                style={{ height: `${h}%` }}
                className={`flex-1 rounded-full transition-all duration-150 ${
                  isPlayed
                    ? "bg-[var(--accent-coral)]"
                    : isPlaying
                    ? "bg-[var(--accent-gold)]/50 group-hover/wave:bg-[var(--accent-gold)]/80"
                    : "bg-[var(--line-hover)] group-hover/wave:bg-[var(--text-dim)]/40"
                }`}
              />
            );
          })}
        </div>
        <span className="font-plex text-[10px] text-[var(--text-dim)]/60 flex-shrink-0 pl-1 select-none">
          {isPlaying ? `${Math.round(audioProgress * 30)}s` : "0:30"}
        </span>
      </div>
    </div>
  );
